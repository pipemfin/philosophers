...
void	*philosophers_thinking(void *philosopher)
{
	...
	while (cycle_of_life)
	{
		...
		custom_usleep(philo->table->sleep_t, philosopher);
...
void	start_thinking(t_table	*table, t_philo **philo, int i)
{
	int	cycle_flag;
	int	status;
	int	parity;
	...
	while (cycle_flag)
	{
		...
		if (i == (table->count + 1 - parity) || (table->count == 1 && i == 2))
			...
			i = 1;
		status = pthread_create(&(philo[i]->thread), NULL,
				philosophers_thinking, philo[i]);
		usleep(10);
		if (status != 0)
